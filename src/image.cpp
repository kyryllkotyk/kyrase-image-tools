...
Image::Image(
    ...
    validate_constructor_input(image_format, width, height, channels, max_color_val);

    const size_t channel_size = get_expected_channel_size(width, height);
    ...
    max_color_val(max_color_val)
{
    ...
    validate_constructor_input(image_format, width, height, 3, max_color_val);

    const size_t expected_size = get_expected_channel_size(width, height);

    validate_channel_size(r_values, expected_size, "R");
    validate_channel_size(g_values, expected_size, "G");
    validate_channel_size(b_values, expected_size, "B");
    ...
    max_color_val(max_color_val)
{
    ...
    validate_constructor_input(image_format, width, height, 4, max_color_val);
    ...
    validate_channel_size(g_values, expected_size, "G");
    validate_channel_size(b_values, expected_size, "B");
    validate_channel_size(a_values, expected_size, "A");
...
void Image::set_r(const vector<uint8_t>& r) {
    const size_t expected_size = get_expected_channel_size(width, height);
    validate_channel_size(r, expected_size, "R");
...
void Image::set_g(const vector<uint8_t>& g) {
    const size_t expected_size = get_expected_channel_size(width, height);
    validate_channel_size(g, expected_size, "G");
...
void Image::set_b(const vector<uint8_t>& b) {
    const size_t expected_size = get_expected_channel_size(width, height);
    validate_channel_size(r, expected_size, "B");
...
void Image::set_a(const vector<uint8_t>& a) {
    ...
    const size_t expected_size = get_expected_channel_size(width, height);
    validate_channel_size(a, expected_size, "A");
...
void Image::validate_constructor_input(
    const string& image_format,
    int width,
    int height,
    int channels,
...
size_t Image::get_expected_channel_size(int width, int height) {
    return static_cast<size_t>(width) * static_cast<size_t>(height);
}

void Image::validate_channel_size(
